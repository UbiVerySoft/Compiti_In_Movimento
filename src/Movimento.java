...
public class Movimento{ // classe per gestire il movimento
    ...
    public void show(float nose_x, float nose_y){
        ...
        if(cont<2){
            ...
            if (!stop) { // stop== false --> personaggio non vicino a un blocco
                //stampo il personaggio nella posizione calcolata dalla nostra equazione
                proceesing.image(personaggio, cordX, cordY, grandezza, larghezza );
                cordYPrec=cordY;
                cordXPrec=cordX;

            }else{ // stop== true --> personaggio vicino a un blocco
